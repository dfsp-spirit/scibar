/// scibar vertical gallery — renders 9 vertical colorbars showcasing diverse features
...
static constexpr int kW = 120, kH = 600;
...
static void render(const char* filename, const Spec& spec, const Style& style) {
    ...
    Rect barRect{45, 50, 30, 500};
    drawColorBar(canvas, barRect, spec, style, Orientation::Vertical);
    drawTicks(canvas, barRect, spec, style, Orientation::Vertical);
    if (style.showSubTicks)
        drawSubTicks(canvas, barRect, spec, style, Orientation::Vertical);
...
int main() {
    // ---- Panel 0: Linear, viridis, forward (default) ----
    {
        ...
        spec.colormap = cmap;
        render("gallery_v_00.ppm", spec, Style::defaultLight());
    }
    ...
    // ---- Panel 1: Linear, viridis, reversed colors ----
    {
        ...
        render("gallery_v_01.ppm", spec, style);
    }
    ...
    // ---- Panel 2: Linear, no sub-ticks, ticks outward ----
    {
        ...
        render("gallery_v_02.ppm", spec, style);
    }
    ...
    // ---- Panel 3: Logarithmic scale ----
    {
        ...
        spec.colormap = cmap;
        render("gallery_v_03.ppm", spec, Style::defaultLight());
    }
    ...
    // ---- Panel 4: Diverging, vik, forward, ticks inward ----
    {
        ...
        render("gallery_v_04.ppm", spec, style);
    }
    ...
    // ---- Panel 5: Diverging, vik, reversed ----
    {
        ...
        auto style = Style::defaultLight();
        style.reverseColors = true;
        render("gallery_v_05.ppm", spec, style);
    }
    ...
    // ---- Panel 6: Categorical, forward (qualitative colormap) ----
    {
        ...
        auto style = Style::defaultLight();
        style.showSubTicks = false;
        render("gallery_v_06.ppm", spec, style);
    }
    ...
    // ---- Panel 7: Categorical, reversed ----
    {
        ...
        style.showSubTicks = false;
        style.reverseColors = true;
        render("gallery_v_07.ppm", spec, style);
    }
    ...
    // ---- Panel 8: Linear, dark theme ----
    {
        ...
        spec.colormap = cmap;
        render("gallery_v_08.ppm", spec, Style::defaultDark());
    }
