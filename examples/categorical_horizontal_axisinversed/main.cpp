// scibar example: axis-inversed categorical horizontal colorbar
//
// Demonstrates scale.inverted on a categorical (qualitative) scale.
// The data direction is flipped: categories run E→A left to right
// instead of A→E. Block colors stay the same — only order reverses.
//
// Compare with:
//   - categorical_horizontal                  (default: A→E, blue→purple)
...
int main() {
    ...
    spec.title      = "Category (axis inverted)";
